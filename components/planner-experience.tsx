...
export function PlannerExperience({
  ...
  const isAiGenerated = resultMeta.source === "openai";
  ...
  return (
    <main className="grid-fade min-h-screen">
      <section className="mx-auto flex w-full max-w-7xl flex-col px-4 py-5 sm:px-6 lg:px-8">
        <header className="glass-panel mb-5 flex flex-col gap-3 rounded-[1.2rem] border border-border-soft px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
          <BrandLogo />
          <div className="flex w-full flex-wrap items-center justify-start gap-2 text-sm text-slate-600 sm:w-auto sm:justify-end">
            <a className="top-nav-link nav-plan" href="#planner">
              Plan a trip
            </a>
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[1fr_0.9fr]">
          <section className="flex flex-col gap-8">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              ...
              <FeatureCard
                ...
                description="Transport and stay links open provider search pages with the trip details carried through where supported."
              />
            </div>
          </section>
          ...
          <section
            id="planner"
            className="glass-panel planner-panel rounded-[1.6rem] border p-5 sm:p-6"
          >
            <div className="planner-panel-header mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
              ...
              <span className="planner-status rounded-full px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.14em]">
                {isAiGenerated ? "AI planner" : "Backup mode"}
              </span>
            </div>

            <form className="space-y-3.5" onSubmit={handleSubmit}>
              <div className="planner-quickstart rounded-[1rem] border p-4">
              ...
              <div className="planner-summary rounded-lg border px-4 py-3.5">
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--color-ink-muted)]">
                  Trip summary
                </p>
              ...
              <div className="planner-disclaimer rounded-lg border p-3.5 text-sm leading-6">
                Prices are estimated ranges generated by the planner. Booking
                buttons open provider search pages so you can compare live
        ...
        <section className="mt-10">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="mt-2 font-serif text-[2rem] text-slate-950 sm:text-[2.4rem]">
                Curated itinerary cards, not a chat transcript
              </h2>
            </div>
            <p className="max-w-md text-sm leading-6 text-[var(--color-ink-muted)]">
              WeekendWanderer turns your dates, budget, and travel vibe into
              short, bookable weekend ideas for European city breaks.
            </p>
          </div>
          ...
          <div className="mb-4 rounded-[1.1rem] border border-border-soft bg-white px-4 py-3.5 text-sm text-slate-600 shadow-[0_12px_28px_rgba(15,23,42,0.05)]">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <p className="text-[11px] uppercase tracking-[0.16em] text-[var(--color-ink-muted)]">
                {isAiGenerated ? "AI-generated" : "Backup recommendations"} •
                updated {formatGeneratedAt(resultMeta)}
              </p>
            </div>
            <p className="mt-2 text-xs leading-5 text-[var(--color-ink-muted)]">
              {isAiGenerated
                ...
                : "AI was unavailable, so WeekendWanderer used its backup planner to rank destination tags, budget fit, and weekend-friendly transit assumptions."}
            </p>
            <p className="mt-1 text-xs leading-5 text-[var(--color-ink-muted)]">
              {resultMeta.bookingLinkPolicy === "provider_search_only"
                ? "Transport and stay buttons open provider route or search pages. Confirm live dates, availability, and prices before booking."
  ...
  return (
    <Link className="brand-lockup" href="/">
      ...
      <span className="brand-copy">
        <span className="brand-wordmark">WeekendWanderer</span>
        <span className="brand-kicker compact-label">
    ...
    >
      ...
      <div className="mt-4 rounded-lg border border-border-soft bg-slate-50 px-3 py-2.5 text-xs leading-5 text-[var(--color-ink-muted)]">
        {meta.bookingLinkPolicy === "provider_search_only"
          ? "Links open provider route or search pages. Confirm live availability and prices on the provider site."
          : trip.disclaimer}
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {options.map((option) => (
          <button
            ...
            className="preset-chip rounded-full px-3 py-1.5 text-sm font-medium"
...
function AuthControls({
  ...
  if (!authEnabled) {
    return (
      <span className="top-nav-link nav-sign-in">
        Auth setup pending
      </span>
    );
  }
  ...
  if (isSignedIn) {
    return (
      <>
        <a className="top-nav-link nav-saved" href="/dashboard">
          Saved trips
        </a>
        <SignOutButton>
          <button className="med-button-light rounded-lg border px-3 py-2 font-semibold">
            Sign out
          </button>
        </SignOutButton>
      </>
  ...
  return (
    <>
      <Link
        className="top-nav-link nav-sign-in"
        href="/sign-in"
        prefetch={false}
      >
        Sign in
      ...
      <Link
        className="top-nav-link nav-sign-up"
        href="/sign-up"
        prefetch={false}
      >
        Sign up
